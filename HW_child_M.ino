...
const uint16_t this_node = 022;     // Address of child_node in Octal format
const uint16_t other_node = 02;     // Address of the parent_node  in Octal format
...
void loop() {
  ...
  while( now - last_sent >= interval  ) {
    last_sent = now;
    Serial.print("Sending to signal post (medium)...");
    payload_t payload = { millis(), packets_sent++,'M', 4444 };
